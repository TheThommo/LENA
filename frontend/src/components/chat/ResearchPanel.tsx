...
export default function ResearchPanel({ messages, persona, activeModes, onClose }: ResearchPanelProps) {
  ...
  const [citeCopiedKey, setCiteCopiedKey] = useState<string | null>(null);
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex flex-col gap-1 flex-shrink-0">
                    <button
                      type="button"
                      onClick={async () => {
                        const ok = await copyTextToClipboard(formatVancouverCitation(c));
                        if (ok) {
                          setCiteCopiedKey(key);
                          window.setTimeout(() => setCiteCopiedKey(null), 2000);
                        }
                      }}
                      className="p-2 rounded-lg transition-colors touch-target flex items-center justify-center text-slate-400 hover:text-slate-700 hover:bg-slate-100"
                      aria-label="Copy citation"
                      title={citeCopiedKey === key ? 'Copied!' : 'Copy Vancouver citation'}
                    >
