...
from app.services import biorxiv, chembl, opentargets, synapse
...
ENRICHMENT_SOURCES = ["chembl", "opentargets", "synapse"]
...
async def search_enrichment_sources(
    ...
    *,
    ...
    """
    from app.core.plan_tiers import AccessTier, enrichment_allowed
    ...
    async def _synapse():
        try:
            if persona_key not in ENRICHMENT_PERSONAS["synapse"]:
                return "synapse", []
            if not enrichment_allowed("synapse", tier):
                return "synapse", []
    ...
    tasks = [_chembl(), _opentargets(), _synapse()]
